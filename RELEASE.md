...
The release version comes from the `v*` tag. The release workflow sets the
Maven version from the tag without its leading `v`, runs Maven verification and
packages the runtime JAR, checksums and build metadata into a draft release.
Review those artifacts before publishing.
